...
in {
  config = lib.mkMerge [
    ...
    # Create wireless.conf for external WPA2 PSK
    (lib.mkIf (hasHost && wifi != null && wifi.mode == "static-wifi") {
      system.activationScripts."snowman-wireless-psk" = ''
        set -euo pipefail

        WIRELESS="/run/secrets/wireless.conf"
        mkdir -p /run/secrets

        echo "# Generated by Snowman" > "$WIRELESS"
        ...
        ${lib.concatStringsSep "\n" (map (netName:
          ...
          in ''
            if [ -f "${config.sops.secrets.${secretName}.path}" ]; then
              ...
              echo "[snowman] WARNING: Secret '${secretName}' missing, skipping." >&2
            fi
          '') wifi.networks)}

        chown root:root "$WIRELESS"
        chmod 0400 "$WIRELESS"
