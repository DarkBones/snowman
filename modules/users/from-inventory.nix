...
let
  ...
  toShell =
    ...
    if lib.isDerivation v then
      ...
      throw "Unknown shell '${toString v}'. Use a nixpkgs attribute name (e.g. 'fish', 'zsh', 'nushell') or an absolute path.";

  shellStrs = lib.unique (map (u: (u.shell or "bash")) (builtins.attrValues users));

  shellPkgs = lib.unique (
    lib.filter lib.isDerivation (map (u: toShell (u.shell or "bash")) (builtins.attrValues users))
  );

  enableFragments = map (
    s:
    lib.mkIf (s != "bash" && lib.hasAttrByPath [ "programs" s "enable" ] options) (
  ...
  keysFor =
    ...
    let
      ...
      filePaths = (lib.optional (u ? sshPubKeyFile) u.sshPubKeyFile) ++ (u.sshPubKeyFiles or [ ]);

      fileKeys = map builtins.readFile filePaths;
    in
    inlineKeys ++ fileKeys;

  sshKeyFileAssertions = lib.mapAttrsToList (
    name: u:
    let
      filePaths = (lib.optional (u ? sshPubKeyFile) u.sshPubKeyFile) ++ (u.sshPubKeyFiles or [ ]);
    in
    ...
    [
      {
        ...
        users.mutableUsers = if hasHost then inv.hosts.${currentHost}.mutableUsers or true else false;
    ...
    ++ [
      {
        ...
        users.users = lib.mapAttrs (
          name: u:
          let
            isSystem = (u ? isSystemUser) && u.isSystemUser;
          in
          (
            {
              uid = u.uid;
              ...
              assertions = [
                {
                  ...
                  message = "Inventory: user ${name} sets BOTH SOPS password and initialPassword. Choose one.";
                }
                {
                  assertion =
                    config.services.openssh.settings.PasswordAuthentication == false || hasSopsPassword || hasInitial;
                  message = "SSH allows passwords but ${name} has neither SOPS password nor initialPassword. Configure one.";
                }
              ];
            }
          ]
...

      {
        ...
        assertions = sshKeyFileAssertions ++ [
          ...
          }
          {
            ...
            message = "Inventory: host ${currentHost} lists unknown users: ${toString unknownUsers}";
          }
          {
            assertion = builtins.length hostUsers > 0;
            message = "Inventory: `${currentHost}.users` must be a non-empty list";
          }
          {
            assertion = lib.all (
              name:
              ...
              in
              hasInitial || hasSopsPassword || hasKeys
